class RoomTypesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @room_type.save
        format.html { redirect_to room_types_url, notice: "[#{@room_type.name}] => #{t('c.created')}" }
  ...
  def update
    ...
    respond_to do |format|
      if @room_type.update_attributes(params[:room_type])
        format.html { redirect_to room_types_url, notice: "[#{@room_type.name}] => #{t('c.updated')}" }
  ...
  def destroy
    ...
    if @room_type.destroy
      respond_to do |format|
        format.html { redirect_to room_types_url, notice: "[#{@room_type.name}] => #{t('c.deleted')}" }
        format.json { head :no_content }
      end
    else
      respond_to do |format|
        format.html { redirect_to room_types_url, alert: "[#{@room_type.name}] => #{t('c.cant_delete')}" }
        format.json { head :no_content }
      end
    end
  end
